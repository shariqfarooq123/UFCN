...
class UFCN:
    def __init__(self):
        self.model = None
        self.model = self.load_ufcn()
        self.func = None
        self.func_layer = None
    ...
    def take_out(self,layer_name,image,average=True):
        ...
        if self.func_layer != layer_name:
            self.func = self.make_func(layer_name)
            self.func_layer = layer_name
        feature_maps = self.func([images])[0]
        if average:
            feature_maps = np.mean(feature_maps,axis=-1)
        else:
            #print 'shape before combination', np.shape(feature_maps)
    ...
    def make_func(self,layer_name, model=None):
        if not model:
            model = self.load_ufcn()
        from keras import backend as K
        if layer_name == 'convolution2d_5':
    ...
    def extract_hyperfeatures(self,img,layers=None):
        if not layers:
            layers = ['activation_1','activation_2','convolution2d_5']
        hyperfeatures = []
        for layer in layers:
            fmaps = list(self.take_out(layer,img,average=False))
            print "shape is ", np.shape(fmaps)
            n, height, width = np.shape(fmaps)
            if height != 1024 & width != 1024:
                for i in xrange(len(fmaps)):
                    fmaps[i] = imresize(fmaps[i], (1024,1024),mode='F')
            print 'now shape is ', np.shape(fmaps)
            hyperfeatures.extend(fmaps)

        return np.array(hyperfeatures)
